	...
	/* **a_inv = inverse of input matrix a (matrix a is destroyed) */
{
	...
	rc = LUDecompose(a, n, col, indx, (fit_double *)NULL);
	if (rc == FALSE)
		{
		for (j = 0; j < n; j++)
			{
	...
	/*   *pd => 1 if number of row interchanges was even, -1 if odd (NULL OK) */
{
	...
	for (i = 0; i < n; i++)
		...
		for (j = 0; j < n; j++)
			{
			if ((temp = (fit_double)fabs(a[i][j])) > big)
				big = temp;
			}
		if (big == 0.0)
			{
	...
	do	{
		...
		for (i = k; i <= l; i++)
			...
			for (j = k; j <= l; j++)
				...
				if (j != i)
					{
					c += (fit_double)fabs(a[j][i]);
					r += (fit_double)fabs(a[i][j]);
					}
				}
...

{
	...
	for (m = kp1; m <= la; m++)
		...
		for (j = m; j <= high; j++)
			{
			if ((fit_double)fabs(a[j][mm1]) > (fit_double)fabs(x))
				{
				x = a[j][mm1];
				i = j;
				}
...

{
	...
	fit_double		norm, p, q, r, s, t, w, x, y, ra, sa, vi, vr, zz, tst1, tst2;
	...
	while (en >= low)
		...
		for (;;)
			...
			for (l = en; l > low; l--)
                            {
				s = (fit_double)fabs(h[l-1][l-1]) + (fit_double)fabs(h[l][l]);
				if (s == 0.0)
                                    s = norm;
				tst1 = s;
				tst2 = tst1 + (fit_double)fabs(h[l][l-1]);
				if (tst2 == tst1)
                                    break;
                            }
			...
			if ((its == 10) || (its == 20))
				...
				s = (fit_double)fabs(h[en][na]) + (fit_double)fabs(h[na][enm2]);
				x = s * 0.75;
				y = x;
				w = s * -0.4375 * s;
                            }
			...
			for (m = enm2; m >= l; m--)
				...
				q = h[m+1][m+1] - zz - r - s;
				r = h[m+2][m+1];
				s = (fit_double)fabs(p) + (fit_double)fabs(q) + (fit_double)fabs(r);
				p /= s;
				q /= s;
				r /= s;
				if (m == l)
                                    break;
				tst1 = (fit_double)fabs(p) * ((fit_double)fabs(h[m-1][m-1]) + (fit_double)fabs(zz) + (fit_double)fabs(h[m+1][m+1]));
				tst2 = tst1 + (fit_double)fabs(h[m][m-1]) * ((fit_double)fabs(q) + (fit_double)fabs(r));
				if (tst2 == tst1)
                                    break;
                            }
			...
			/* fit_double qr step involving rows l to en and columns m to en */
			for (k = m; k <= na; k++)
                            {
				notlas = (k != na);
				if (k != m)
					...
					if (notlas)
						r = h[k+2][k-1];
					x = (fit_double)fabs(p) + (fit_double)fabs(q) + (fit_double)fabs(r);
					if (x == 0.0)
                                            continue;
					p /= x;
					q /= x;
				...
				s = D_sign((fit_double)sqrt(p*p + q*q + r*r), p);
				if (k != m)
                                    h[k][k-1] = -s * x;
				else if (l != m)
                                    h[k][k-1] = -h[k][k-1];
		...
		if (twoRoots)
			...
			p = (y - x) / 2.0;
			q = p * p + w;
			zz = (fit_double)sqrt((fit_double)fabs(q));
			h[en][en] = x + t;
			x = h[en][en];
			h[na][na] = y + t;
			...
			*/
			if (q >= -1e-12)
				...
				wi[en] = 0.0;
				x = h[en][na];
				s = (fit_double)fabs(x) + (fit_double)fabs(zz);
				p = x / s;
				q = zz / s;
				r = (fit_double)sqrt(p*p + q*q);
				p /= r;
				q /= r;
				/* row modification */
				for (j = na; j < n; j++)
	...
	for (en = n - 1; en >= 0; en--)
		...
		if (q < -1e-12)
			...
			if ((fit_double)fabs(h[en][na]) > (fit_double)fabs(h[na][en]))
				{
				h[na][na] = q / h[en][na];
				...
				for (i = enm2; i >= 0; i--)
					...
					if (wi[i] < 0.0)
						...
						if (wi[i] == 0.0)
							...
							if ((vr == 0.0) && (vi == 0.0))
								{
								tst1 = norm * ((fit_double)fabs(w) + (fit_double)fabs(q) + (fit_double)fabs(x) + (fit_double)fabs(y) + (fit_double)fabs(zz));
								vr = tst1;
								do	{
									vr *= .01;
									tst2 = tst1 + vr;
							...
							if ((fit_double)fabs(x) > (fit_double)fabs(zz) + (fit_double)fabs(q))
								{
						...
						/* overflow control */
						tst1 = (fit_double)fabs(h[i][na]);
						tst2 = (fit_double)fabs(h[i][en]);
						t = MAX(tst1, tst2);
						if (t != 0.0)
							{
							tst1 = t;
				...
				for (i = na; i >= 0; i--)
					...
					if (wi[i] < 0.0)
						...
						if (wi[i] == 0.0)
							...
							t = (x * s - zz * r) / q;
							h[i][en] = t;
							if ((fit_double)fabs(x) > (fit_double)fabs(zz))
								h[i+1][en] = (-r - w * t) / x;
							else
								h[i+1][en] = (-s - y * t) / zz;
							}
...
{
	fit_double		s, ais, bis, ars, brs;

	s = (fit_double)fabs(br) + (fit_double)fabs(bi);
	ars = ar / s;
	ais = ai / s;
	brs = br / s;
	bis = bi / s;
